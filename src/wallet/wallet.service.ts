...
import { WalletTransactions, TransactionType } from './entities/transaction.entity';
...
export class WalletService {
  ...
  async getTransactions(userId: number) {
    const wallet = await this.walletRepo.findOne({ where: { user: { id: userId } }, relations: ['transactions'] });
    if (!wallet) throw new NotFoundException('Wallet not found');
